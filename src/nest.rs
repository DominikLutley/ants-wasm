...
use web_sys::{WebGl2RenderingContext, WebGlProgram, WebGlUniformLocation, WebGlVertexArrayObject, WebGlBuffer};
use wasm_bindgen::JsCast;
use crate::{functions::{compile_shader, link_program, draw_points}, consts::NEST_SIZE};
...
pub struct NestRenderer {
    ...
    u_nest_size_location: Option<WebGlUniformLocation>,
...
impl NestRenderer {
    pub fn new(gl: &WebGl2RenderingContext, width: f32, height: f32) -> Result<Self, JsValue> {
        ...
        let u_nest_size_location = gl.get_uniform_location(&program, "u_nest_size");
        ...
        let vertices = [width / 2.0, height / 2.0];
    ...
    pub fn render(&self, gl: &WebGl2RenderingContext) {
        ...
        gl.uniform4fv_with_f32_array(self.u_color_location.as_ref(), &[1.0, 0.5, 1.0, 1.0]);
        gl.uniform2f(self.u_resolution_location.as_ref(), self.width, self.height);
        gl.uniform1f(self.u_nest_size_location.as_ref(), NEST_SIZE);
